...
// --- Back Button Handler ---
// Componente que gerencia o botão voltar para evitar saída acidental
const BackButtonHandler = () => {
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    // Ao montar, empurra um estado no histórico para garantir que haja algo para "voltar"
    // isso cria uma "barreira" antes de sair do app
    window.history.pushState(null, document.title, window.location.href);

    const handlePopState = (event: PopStateEvent) => {
      // Quando o usuário aperta voltar
      if (location.pathname !== '/') {
        // Se não estiver na home, vai para a home
        navigate('/');
      } else {
        // Se estiver na home, empurra o estado novamente para impedir a saída
        // O usuário precisaria apertar voltar muito rápido duas vezes (comportamento padrão de apps nativos)
        // ou aqui apenas bloqueamos mantendo na home
        window.history.pushState(null, document.title, window.location.href);
      }
    };
